...
![观察者模式](设计模式：观察者模式/vue.png)

在 vue 源码中， 初始化data 的时候， vue 会通过一个 `defineReactive` 函数， 这个函数是实现当数据更新时相关依赖进行更新的关键， 这个函数里面对于数据的 `getter` 和 `setter` 函数内进行了一些处理， 当触发 `getter` 函数的时候会进行依赖收集， 当 触发 `getter` 函数（设置）的时候会通知相关依赖进行更新。
...
这里 `dep` 是用来维护者一组的观察者， 通过 `dep.depend` 方法收集到依赖， 然后通过 `dep.notify()` 来通知变动。
